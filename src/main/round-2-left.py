...
AVOID_STEER_LEFT = 45       # Steer Left for Green Block (Pass on Right)
AVOID_STEER_RIGHT = 135     # Steer Right for Red Block (Pass on Left)
...
try:
    print("\n==================================================")
    print("🚀 MC-SQUARED: ROUND 2 LEFT INNER FOLLOW + AVOIDANCE")
    print("==================================================\n")
    ...
    while True:
        ...
        if is_turning:
            if current_time - turn_start_time < TURN_DURATION:
                ...
                print(f"🌀 EXECUTING CORNER TURN ({current_time - turn_start_time:.2f}s)", end="\r")
                time.sleep(dt)
                continue
            else:
        ...
        if not in_cooldown and (dist is None or dist >= CORNER_SPIKE_THRESH):
            ...
            print(f"\n🚨 CORNER SPIKE ({dist}) -> Hard Left Turn Initiated!")
            send_to_arduino(BASE_THROTTLE, STEERING_MAX)
            continue
